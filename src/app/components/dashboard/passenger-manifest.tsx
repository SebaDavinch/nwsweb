...
const YAKUT_AIRPORTS = new Set(["UEEE","UERR","UERP","UEBT","UEJN","UEMM","UENS"]);
const DAGESTAN_AIRPORTS = new Set(["URML"]);
const CHECHEN_AIRPORTS = new Set(["URMG"]);

function icaoToPools(icao: string): [string, number][] {
  const code = icao.toUpperCase();
  if (KAZAN_AIRPORTS.has(code)) return [["tatar", 0.75], ["russian", 0.25]];
  if (YAKUT_AIRPORTS.has(code) || code.startsWith("UE")) return [["yakut", 0.7], ["russian", 0.3]];
  if (DAGESTAN_AIRPORTS.has(code)) return [["dagestani", 0.75], ["russian", 0.25]];
  if (CHECHEN_AIRPORTS.has(code)) return [["chechen", 0.75], ["russian", 0.25]];

  const p2 = code.slice(0, 2);
  const p1 = code.slice(0, 1);

  if (p2 === "UR") return [["chechen", 0.2], ["dagestani", 0.2], ["russian", 0.6]];
...
function generateManifest(bookingId: number, depIcao: string, arrIcao: string, paxCount: number): Passenger[] {
  ...
  return seats.map((seat) => {
    ...
    const isChild = rng() < 0.18;

    const birthYear = isChild
      ? currentYear - 2 - Math.floor(rng() * 16)
      : currentYear - 18 - Math.floor(rng() * 58);
