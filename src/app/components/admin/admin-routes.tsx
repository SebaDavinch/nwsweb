  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500">Operational route catalog with hub and priority metadata.</p>
      </div>
        ...
        <CardContent className="space-y-4 p-4">
          <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
            <div className="relative w-full xl:max-w-sm">
              ...
              <Input className="pl-9" placeholder="Search routes..." value={search} onChange={(event) => setSearch(event.target.value)} />
            </div>
          ...
          <div className="rounded-md border border-gray-200 bg-white">
            <Table>
              ...
              <TableBody>
                {isLoading ? (
                  <TableRow>
                    <TableCell colSpan={5} className="py-10 text-center text-gray-500">Loading routes...</TableCell>
                  </TableRow>
                ) : filteredRoutes.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={5} className="py-10 text-center text-gray-500">No routes found.</TableCell>
                  </TableRow>
                ) : (
                  filteredRoutes.map((route) => (
                    <TableRow key={route.id}>
                      ...
                      <TableCell>
                        ...
                        <div className="text-xs text-gray-500">{route.duration} · {route.fleetIds?.length || 0} fleet refs</div>
                      </TableCell>
                      <TableCell>
                        ...
                        <div className="mt-1 text-xs text-gray-500">{route.meta?.hubLabel || "No hub assigned"}</div>
                      </TableCell>
                      <TableCell>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>Edit Route Metadata</DialogTitle>
          </DialogHeader>
