...
// DIAGNOSTIC LOGGING
console.log('=== DISCORD BOT STARTUP DIAGNOSTICS ===');
console.log('[ENV] Checking environment variables...');
console.log(`[ENV] DISCORD_BOT_TOKEN: ${token ? '✓ SET (' + token.substring(0, 20) + '...)' : '✗ MISSING'}`);
console.log(`[ENV] DISCORD_CLIENT_ID: ${clientId ? '✓ SET (' + clientId + ')' : '✗ MISSING'}`);
console.log(`[ENV] DISCORD_GUILD_ID: ${guildId ? '✓ SET (' + guildId + ')' : '⚠ NOT SET (will register globally)'}`);
console.log(`[ENV] VAMSYS_CLIENT_ID: ${vamsysClientId ? '✓ SET' : '⚠ NOT SET'}`);
console.log(`[ENV] VAMSYS_CLIENT_SECRET: ${vamsysClientSecret ? '✓ SET' : '⚠ NOT SET'}`);
console.log('[ENV] Environment loading completed');
...
let photoLikesNotificationTimer = null;
let flightCompletionNotificationTimer = null;
let flightReviewNotificationTimer = null;
let contentAutoPublishTimer = null;
let lastKnownPilotCount = null;
let defaultLanguage = (process.env.DEFAULT_LANGUAGE || 'en').toLowerCase().substring(0, 2);

const photoLikesSeen = new Map();
const flightCompletionSeen = new Map();
const flightReviewSeen = new Map();
const liveFeedMessages = new Map();

const PILOT_LANGUAGES = {
  'en': 'English',
  'ru': 'Русский',
};
...
const LANGUAGE_STRINGS = {
  'en': {
    PHOTO_LIKE_TITLE: 'Photo Like',
    PHOTO_LIKE_DESC: 'Someone liked your screenshot!',
    PHOTO_LIKE_LIKER: 'Liker',
    PHOTO_LIKE_TITLE_FIELD: 'Screenshot',
    PHOTO_LIKE_LIKES_COUNT: 'Total Likes',
    FLIGHT_COMPLETION_TITLE: 'Flight Completed',
    FLIGHT_COMPLETION_DESC: 'Your flight has been completed successfully.',
    FLIGHT_COMPLETION_ROUTE: 'Route',
    FLIGHT_COMPLETION_AIRCRAFT: 'Aircraft',
    FLIGHT_COMPLETION_REGISTRATION: 'Registration',
    FLIGHT_COMPLETION_FLIGHT_TIME: 'Flight Time',
    FLIGHT_COMPLETION_GATE_OUT: 'Gate Out',
    FLIGHT_COMPLETION_TAKEOFF: 'Takeoff',
    FLIGHT_COMPLETION_LANDING: 'Landing',
    FLIGHT_COMPLETION_GATE_IN: 'Gate In',
    FLIGHT_COMPLETION_VS: 'Max V/S',
    FLIGHT_COMPLETION_G_FORCE: 'Max G-Force',
    FLIGHT_COMPLETION_POINTS: 'Points Earned',
    FLIGHT_COMPLETION_COINS: 'Coins Earned',
    FLIGHT_COMPLETION_EVENT: 'Event Participation',
    FLIGHT_COMPLETION_EVENT_YES: 'Yes, participated in **{eventName}**',
    FLIGHT_COMPLETION_NO_EVENT: 'No',
    FLIGHT_REVIEW_TITLE: 'PIREP Under Review',
    FLIGHT_REVIEW_DESC: 'Your PIREP has been submitted for review.',
    FLIGHT_REVIEW_STATUS: 'Review Status',
    FLIGHT_REVIEW_SUBMITTED: 'Submitted for Review',
  },
  'ru': {
    PHOTO_LIKE_TITLE: 'Лайк на фотку',
    PHOTO_LIKE_DESC: 'Кто-то поставил лайк на вашу фотографию!',
    PHOTO_LIKE_LIKER: 'От кого',
    PHOTO_LIKE_TITLE_FIELD: 'Скриншот',
    PHOTO_LIKE_LIKES_COUNT: 'Всего лайков',
    FLIGHT_COMPLETION_TITLE: 'Полёт завершён',
    FLIGHT_COMPLETION_DESC: 'Ваш полёт успешно завершён.',
    FLIGHT_COMPLETION_ROUTE: 'Маршрут',
    FLIGHT_COMPLETION_AIRCRAFT: 'Воздушное судно',
    FLIGHT_COMPLETION_REGISTRATION: 'Борт. номер',
    FLIGHT_COMPLETION_FLIGHT_TIME: 'Время полёта',
    FLIGHT_COMPLETION_GATE_OUT: 'Отход от стоянки',
    FLIGHT_COMPLETION_TAKEOFF: 'Взлёт',
    FLIGHT_COMPLETION_LANDING: 'Посадка',
    FLIGHT_COMPLETION_GATE_IN: 'Рулёж',
    FLIGHT_COMPLETION_VS: 'Макс. вертик. скорость',
    FLIGHT_COMPLETION_G_FORCE: 'Макс. перегрузка',
    FLIGHT_COMPLETION_POINTS: 'Начислено баллов',
    FLIGHT_COMPLETION_COINS: 'Начислено монеток',
    FLIGHT_COMPLETION_EVENT: 'Участие в ивенте',
    FLIGHT_COMPLETION_EVENT_YES: 'Да, участвовал в **{eventName}**',
    FLIGHT_COMPLETION_NO_EVENT: 'Нет',
    FLIGHT_REVIEW_TITLE: 'PIREP на рассмотрении',
    FLIGHT_REVIEW_DESC: 'Ваш PIREP отправлен на проверку.',
    FLIGHT_REVIEW_STATUS: 'Статус проверки',
    FLIGHT_REVIEW_SUBMITTED: 'На рассмотрении',
  },
};
...
if (!token || !clientId) {
  console.error('Missing DISCORD_BOT_TOKEN or DISCORD_CLIENT_ID in environment variables (.env).');
  process.exit(1);
}
...
async function registerCommands() {
  console.log(`[REGISTER] Starting command registration...`);
  console.log(`[REGISTER] Token available: ${token ? 'Yes' : 'NO - THIS WILL FAIL'}`);
  console.log(`[REGISTER] ClientId available: ${clientId ? 'Yes (' + clientId + ')' : 'NO - THIS WILL FAIL'}`);
  console.log(`[REGISTER] Commands to register: ${Array.isArray(commands) ? commands.length : '0'}`);
  
  const rest = new REST({ version: '10' }).setToken(token);

  if (guildId) {
    console.log(`[REGISTER] Registering ${commands.length} commands for guild: ${guildId}`);
    await rest.put(Routes.applicationGuildCommands(clientId, guildId), {
      body: commands,
    });
    console.log(`[REGISTER] ✓ Slash commands registered for guild ${guildId}`);
  } else {
    console.log(`[REGISTER] Registering ${commands.length} commands globally (may take 1+ hour to appear)`);
    await rest.put(Routes.applicationCommands(clientId), { body: commands });
    console.log(`[REGISTER] ✓ Slash commands registered globally`);
  }
}
...
async function registerCommandsWithRetry({ attempts = 5, baseDelayMs = 5000 } = {}) {
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      ...
      console.error(
        `Slash command registration failed (attempt ${attempt}/${attempts}, status ${status}): ${message}`
      );
...
function cloneNotificationSettings(value = DEFAULT_NOTIFICATION_SETTINGS) {
  return {
    ...
    language:
      PILOT_LANGUAGES[String(value?.language || '').toLowerCase().substring(0, 2)]
        ? String(value?.language || '').toLowerCase().substring(0, 2)
...
function getPilotLanguage(candidate = {}) {
  const { preferences } = getPilotPreferences(candidate);
  const lang = String(preferences?.notifications?.language || defaultLanguage || 'en').toLowerCase().substring(0, 2);
  return PILOT_LANGUAGES[lang] ? lang : 'en';
}

function getTranslation(stringKey, language = 'en') {
  const lang = PILOT_LANGUAGES[language] ? language : 'en';
  const strings = LANGUAGE_STRINGS[lang] || LANGUAGE_STRINGS['en'];
  return strings[stringKey] || LANGUAGE_STRINGS['en'][stringKey] || stringKey;
}

function formatTranslation(stringKey, params = {}, language = 'en') {
  let text = getTranslation(stringKey, language);
...
async function loadCurrentAlerts({ limit = 25 } = {}) {
  const payload = await vamsysFetchAllPages('/alerts?page[size]=50');
  return (Array.isArray(payload) ? payload : [])
    .map((alert) => ({
      id: Number(alert?.id || 0) || 0,
      title: String(alert?.title || 'Operational Alert').trim() || 'Operational Alert',
      summary:
        String(alert?.summary || alert?.excerpt || '').trim() ||
        String(alert?.content || '').trim().slice(0, 240),
      content: String(alert?.content || alert?.summary || '').trim(),
      type: String(alert?.type || 'info').trim() || 'info',
      startShowing: String(alert?.start_showing || alert?.startShowing || '').trim() || null,
      stopShowing: String(alert?.stop_showing || alert?.stopShowing || '').trim() || null,
...
function buildAutoPublishedNotamEmbed(notam = {}) {
  const title = String(notam?.title || 'Untitled NOTAM').trim() || 'Untitled NOTAM';
  const content = truncateText(notam?.content || 'No content provided.', 3200) || 'No content provided.';
  const readUrl = String(notam?.url || `${websiteBaseUrl}/dashboard?tab=notams`).trim();

  return new EmbedBuilder()
    .setColor(0xdc2626)
    .setDescription(
      [
        '**ATTENTION! NEW NOTAM! MANDADORY TO CHECK**',
        '',
        `**${title}**`,
        '',
        content,
        '',
        `[click here to read](${readUrl})`,
      ].join('\n')
    )
    .setTimestamp(new Date());
}

function buildAutoPublishedAlertEmbed(alert = {}) {
  return new EmbedBuilder()
    .setColor(0xf59e0b)
    .setTitle(`Alert: ${String(alert?.title || 'Operational Alert').trim() || 'Operational Alert'}`)
    .setDescription(
      truncateText(
        String(alert?.content || alert?.summary || 'No content provided.').trim() || 'No content provided.',
        3500
      )
    )
    .addFields(
      { name: 'Type', value: toTitleCaseWords(String(alert?.type || 'info')), inline: true },
      { name: 'Published', value: formatUtcDate(alert?.createdAt || alert?.updatedAt), inline: true }
    )
    .setTimestamp(new Date());
}

function getContentAutoPublishState() {
  const store = loadBotAuthStore() || {};
  const state = store.contentAutoPublishState && typeof store.contentAutoPublishState === 'object'
    ? store.contentAutoPublishState
    : {};

  return {
    store,
    state: {
      initializedAt: String(state?.initializedAt || '').trim() || null,
      notamIds: Array.isArray(state?.notamIds)
        ? state.notamIds.map((item) => Number(item || 0) || 0).filter((item) => item > 0)
        : [],
      alertIds: Array.isArray(state?.alertIds)
        ? state.alertIds.map((item) => Number(item || 0) || 0).filter((item) => item > 0)
        : [],
...
function setContentAutoPublishState(nextState = {}) {
  const { store } = getContentAutoPublishState();
  store.contentAutoPublishState = {
    initializedAt: String(nextState?.initializedAt || '').trim() || new Date().toISOString(),
    notamIds: Array.from(
      new Set(
        (Array.isArray(nextState?.notamIds) ? nextState.notamIds : [])
    ...
    alertIds: Array.from(
      new Set(
        (Array.isArray(nextState?.alertIds) ? nextState.alertIds : [])
          .map((item) => Number(item || 0) || 0)
          .filter((item) => item > 0)
      )
    ).slice(-500),
  ...
  saveBotAuthStore(store);
  return store.contentAutoPublishState;
}

function getConfiguredChannelIdForContent(kind) {
  const remoteChannel = String(getSyncedDiscordBotSettings()?.channels?.[kind] || '').trim();
  if (remoteChannel) {
    return remoteChannel;
  }
  if (kind === 'alerts') {
    return alertFeedChannelId;
  }
  if (kind === 'notams') {
    return notamFeedChannelId;
  }
  return '';
}

function isContentAutoPublishEnabled(kind) {
  const syncSettings = getSyncedDiscordBotSettings()?.sync;
  const value = syncSettings && typeof syncSettings === 'object' ? syncSettings[kind] : undefined;
  return value !== false;
}

async function notifyAutoPublishedContent() {
  if (!client.isReady() || !hasVamsysCredentials()) {
    return;
  }

  const [notamChannelId, alertChannelId] = [
    getConfiguredChannelIdForContent('notams'),
    getConfiguredChannelIdForContent('alerts'),
  ];

  const shouldHandleNotams = Boolean(notamChannelId && isContentAutoPublishEnabled('notams'));
  const shouldHandleAlerts = Boolean(alertChannelId && isContentAutoPublishEnabled('alerts'));
  if (!shouldHandleNotams && !shouldHandleAlerts) {
    return;
  }

  const [notams, alerts] = await Promise.all([
    shouldHandleNotams ? loadCurrentNotams({ limit: 50 }).catch(() => []) : Promise.resolve([]),
    shouldHandleAlerts ? loadCurrentAlerts({ limit: 50 }).catch(() => []) : Promise.resolve([]),
  ]);

  const { state } = getContentAutoPublishState();
  const nextNotamIds = new Set(state.notamIds);
  const nextAlertIds = new Set(state.alertIds);
  const isFirstRun = !state.initializedAt;

  if (isFirstRun) {
    for (const notam of notams) {
      if ((Number(notam?.id || 0) || 0) > 0) {
        nextNotamIds.add(Number(notam.id));
      }
    }
    for (const alert of alerts) {
      if ((Number(alert?.id || 0) || 0) > 0) {
        nextAlertIds.add(Number(alert.id));
      }
    }
    setContentAutoPublishState({
      initializedAt: new Date().toISOString(),
      notamIds: [...nextNotamIds],
      alertIds: [...nextAlertIds],
    });
    return;
  }

  if (shouldHandleNotams) {
    const channel = await client.channels.fetch(notamChannelId).catch(() => null);
    if (channel?.isTextBased()) {
      const toPublish = [...notams]
        .filter((item) => !nextNotamIds.has(Number(item?.id || 0) || 0))
        .sort((left, right) => {
          const leftTime = Date.parse(String(left?.createdAt || ''));
          const rightTime = Date.parse(String(right?.createdAt || ''));
      ...
      for (const notam of toPublish) {
        await channel.send({ embeds: [buildAutoPublishedNotamEmbed(notam)] }).catch(() => null);
        nextNotamIds.add(Number(notam.id));
      }
    }
  }
  ...
  if (shouldHandleAlerts) {
    ...
    if (channel?.isTextBased()) {
      const toPublish = [...alerts]
        .filter((item) => !nextAlertIds.has(Number(item?.id || 0) || 0))
        .sort((left, right) => {
          const leftTime = Date.parse(String(left?.createdAt || left?.updatedAt || ''));
          const rightTime = Date.parse(String(right?.createdAt || right?.updatedAt || ''));
          return (Number.isFinite(leftTime) ? leftTime : 0) - (Number.isFinite(rightTime) ? rightTime : 0);
      ...
      for (const alert of toPublish) {
        await channel.send({ embeds: [buildAutoPublishedAlertEmbed(alert)] }).catch(() => null);
        nextAlertIds.add(Number(alert.id));
      }
    }
  }
...
function buildPhotoLikeEmbed({ liker = null, photoTitle = null, totalLikes = 0, language = 'en' } = {}) {
  const title = getTranslation('PHOTO_LIKE_TITLE', language);
  const desc = getTranslation('PHOTO_LIKE_DESC', language);
  const likerLabel = getTranslation('PHOTO_LIKE_LIKER', language);
  const titleLabel = getTranslation('PHOTO_LIKE_TITLE_FIELD', language);
  const likesLabel = getTranslation('PHOTO_LIKE_LIKES_COUNT', language);
  ...
  if (liker) {
    embed.addFields({ name: likerLabel, value: String(liker).trim() || 'Anonymous', inline: true });
  }
  if (photoTitle) {
    embed.addFields({ name: titleLabel, value: String(photoTitle).trim() || 'Untitled', inline: true });
  }
  embed.addFields({ name: likesLabel, value: String(totalLikes || 0), inline: true });
  
  return embed.setTimestamp(new Date());
}

function buildFlightCompletionEmbed({
  departure = null,
  arrival = null,
  aircraft = null,
  registration = null,
  ...
  const title = getTranslation('FLIGHT_COMPLETION_TITLE', language);
  const desc = getTranslation('FLIGHT_COMPLETION_DESC', language);
  const routeLabel = getTranslation('FLIGHT_COMPLETION_ROUTE', language);
  const aircraftLabel = getTranslation('FLIGHT_COMPLETION_AIRCRAFT', language);
  const registrationLabel = getTranslation('FLIGHT_COMPLETION_REGISTRATION', language);
  const flightTimeLabel = getTranslation('FLIGHT_COMPLETION_FLIGHT_TIME', language);
  const gateOutLabel = getTranslation('FLIGHT_COMPLETION_GATE_OUT', language);
  const takeoffLabel = getTranslation('FLIGHT_COMPLETION_TAKEOFF', language);
  const landingLabel = getTranslation('FLIGHT_COMPLETION_LANDING', language);
  const gateInLabel = getTranslation('FLIGHT_COMPLETION_GATE_IN', language);
  const vsLabel = getTranslation('FLIGHT_COMPLETION_VS', language);
  const gForceLabel = getTranslation('FLIGHT_COMPLETION_G_FORCE', language);
  const pointsLabel = getTranslation('FLIGHT_COMPLETION_POINTS', language);
  const coinsLabel = getTranslation('FLIGHT_COMPLETION_COINS', language);
  const eventLabel = getTranslation('FLIGHT_COMPLETION_EVENT', language);
  ...
  if (departure && arrival) {
    embed.addFields({ name: routeLabel, value: `${departure} → ${arrival}`, inline: false });
  }
  if (aircraft) {
    embed.addFields({ name: aircraftLabel, value: aircraft, inline: true });
  }
  if (registration) {
    embed.addFields({ name: registrationLabel, value: registration, inline: true });
  }
  
  const times = [];
  if (gateOut) times.push(`${gateOutLabel}: ${formatUtcTime(gateOut)}`);
  if (takeoff) times.push(`${takeoffLabel}: ${formatUtcTime(takeoff)}`);
  if (landing) times.push(`${landingLabel}: ${formatUtcTime(landing)}`);
  if (gateIn) times.push(`${gateInLabel}: ${formatUtcTime(gateIn)}`);
  
  if (times.length > 0) {
    embed.addFields({ name: flightTimeLabel, value: times.join('\n'), inline: false });
  }
  
  const stats = [];
  if (maxVS !== null) stats.push(`${vsLabel}: ${maxVS} ft/min`);
  if (maxGForce !== null) stats.push(`${gForceLabel}: ${maxGForce}G`);
  if (pointsEarned > 0) stats.push(`${pointsLabel}: ${pointsEarned}`);
  if (coinsEarned > 0) stats.push(`${coinsLabel}: ${coinsEarned}`);
  
  if (stats.length > 0) {
    embed.addFields({ name: eventLabel, value: stats.join(' · '), inline: false });
  }
  
  if (eventName) {
    const eventText = formatTranslation('FLIGHT_COMPLETION_EVENT_YES', { eventName }, language);
    embed.addFields({ name: eventLabel, value: eventText, inline: false });
  }
  
  return embed.setTimestamp(new Date());
}

function buildFlightReviewEmbed({ pirepStatus = null, language = 'en' } = {}) {
  const title = getTranslation('FLIGHT_REVIEW_TITLE', language);
  const desc = getTranslation('FLIGHT_REVIEW_DESC', language);
  const statusLabel = getTranslation('FLIGHT_REVIEW_STATUS', language);
  const statusValue = getTranslation('FLIGHT_REVIEW_SUBMITTED', language);
  
  return new EmbedBuilder()
    .setColor(0xf59e0b)
    .setTitle(title)
    .setDescription(desc)
    .addFields({ name: statusLabel, value: statusValue, inline: true })
    .setTimestamp(new Date());
}
...
async function notifyPhotoLikes() {
  if (!client.isReady() || !hasVamsysCredentials()) {
    return;
  }
  ...
  for (const binding of bindings) {
    const preferences = getPilotPreferences(binding).preferences;
    if (!preferences.notifications.channels.discord || !preferences.notifications.notificationTypes.photoLikes) {
      continue;
    }

    try {
      const profile = await getPilotProfileData(binding.discordUserId).catch(() => null);
      if (!profile) {
        continue;
      }

      const pilotId = Number(binding.vamsysUserId || 0) || 0;
      if (pilotId <= 0) {
        continue;
      }

      const seen = photoLikesSeen.get(binding.discordUserId) || new Set();
      let foundNewLike = false;

      if (Array.isArray(profile.gallery) && profile.gallery.length > 0) {
        for (const photo of profile.gallery) {
          const photoId = Number(photo?.id || 0) || 0;
          if (photoId <= 0) {
            continue;
          }

          const likeCount = Array.isArray(photo?.likes) ? photo.likes.length : 0;
          const likeCountKey = `${photoId}:${likeCount}`;

          if (!seen.has(likeCountKey) && likeCount > 0) {
            const language = getPilotLanguage(binding);
            const latestLiker = Array.isArray(photo.likes) && photo.likes.length > 0
              ? String(photo.likes[0]?.name || photo.likes[0] || 'Someone').trim()
              : 'Someone';

            const user = await client.users.fetch(binding.discordUserId).catch(() => null);
            if (user) {
              const embed = buildPhotoLikeEmbed({
                liker: latestLiker,
                photoTitle: String(photo?.title || '').trim() || 'Untitled',
            ...
            seen.add(likeCountKey);
          }
        }
      }

      if (foundNewLike || seen.size > 0) {
        photoLikesSeen.set(binding.discordUserId, seen);
      }
    } catch (error) {
      console.error('Photo likes notification failed for user:', binding.discordUserId, error?.message || error);
    }
  }
}

async function notifyFlightCompletion() {
  if (!client.isReady() || !hasVamsysCredentials()) {
    return;
  }
  ...
  for (const binding of bindings) {
    const preferences = getPilotPreferences(binding).preferences;
    if (!preferences.notifications.channels.discord || !preferences.notifications.notificationTypes.flightCompletion) {
      continue;
    }
    ...
    try {
      ...
      const pireps = await vamsysFetch(`/pireps?page[size]=50&filter[pilot_id]=${pilotId}&sort=-created_at`).catch(() => ({ data: [] }));
      const recentPireps = Array.isArray(pireps?.data) ? pireps.data.slice(0, 5) : [];

      const seen = flightCompletionSeen.get(binding.discordUserId) || new Set();

      for (const pirep of recentPireps) {
        const pirepId = Number(pirep?.id || 0) || 0;
        if (pirepId <= 0 || seen.has(pirepId)) {
          continue;
        }

        const status = String(pirep?.status || '').trim().toLowerCase();
        if (ACCEPTED_PIREP_STATUSES.has(status)) {
          const language = getPilotLanguage(binding);
          const departure = String(pirep?.departure_id || pirep?.departure?.icao || '—').trim() || '—';
          const arrival = String(pirep?.arrival_id || pirep?.arrival?.icao || '—').trim() || '—';
          const aircraft = String(pirep?.aircraft?.name || pirep?.aircraft_type || '—').trim() || '—';
          const registration = String(pirep?.aircraft_registration || pirep?.aircraft?.registration || '—').trim() || '—';

          const gateOut = pirep?.started_at || pirep?.gate_out_time || null;
          const takeoff = pirep?.took_off_at || pirep?.takeoff_time || null;
          const landing = pirep?.landed_at || pirep?.landing_time || null;
          const gateIn = pirep?.finished_at || pirep?.gate_in_time || null;

          const maxVS = Number(pirep?.max_vertical_speed || pirep?.max_vs || 0) || null;
          const maxGForce = Number(pirep?.max_g_force || pirep?.max_g || 0) || null;
          const pointsEarned = Number(pirep?.points || pirep?.score || 0) || 0;
          const coinsEarned = Number(pirep?.balance_added || pirep?.coins || 0) || 0;
          const eventName = String(pirep?.event_name || pirep?.event || '').trim() || null;

          const user = await client.users.fetch(binding.discordUserId).catch(() => null);
          if (user) {
            const embed = buildFlightCompletionEmbed({
              departure,
              arrival,
              aircraft,
              registration,
      ...
      if (seen.size > 0) {
        flightCompletionSeen.set(binding.discordUserId, seen);
      }
    } catch (error) {
      console.error('Flight completion notification failed for user:', binding.discordUserId, error?.message || error);
    }
  }
}

async function notifyFlightReviewStatus() {
  if (!client.isReady() || !hasVamsysCredentials()) {
    return;
  }
  ...
  for (const binding of bindings) {
    const preferences = getPilotPreferences(binding).preferences;
    if (!preferences.notifications.channels.discord || !preferences.notifications.notificationTypes.flightReview) {
      continue;
    }
    ...
    try {
      ...
      const pireps = await vamsysFetch(`/pireps?page[size]=50&filter[pilot_id]=${pilotId}&sort=-created_at`).catch(() => ({ data: [] }));
      const recentPireps = Array.isArray(pireps?.data) ? pireps.data.slice(0, 10) : [];

      const seen = flightReviewSeen.get(binding.discordUserId) || new Set();
      ...
      for (const pirep of recentPireps) {
        ...
        if (status === 'pending' || status === 'under_review') {
          ...
          const user = await client.users.fetch(binding.discordUserId).catch(() => null);
          if (user) {
            const embed = buildFlightReviewEmbed({ pirepStatus: status, language });
            await user.send({ embeds: [embed] }).catch(() => null);
          }
          seen.add(pirepId);
        }
      ...
      if (seen.size > 0) {
        flightReviewSeen.set(binding.discordUserId, seen);
      }
    } catch (error) {
      console.error('Flight review notification failed for user:', binding.discordUserId, error?.message || error);
    }
  }
}
...
async function getRegisteredPilotsCount() {
  try {
    ...
    const count = Number(
      general?.data?.pilots?.current ??
      general?.data?.pilots?.total ??
      general?.data?.current_pilots ??
      general?.data?.total_pilots
    );
  ...
  try {
    ...
    const total = Number(pilots?.meta?.total ?? pilots?.meta?.count ?? pilots?.meta?.pagination?.total);
    if (Number.isFinite(total) && total >= 0) {
      return total;
    }

    const listCount = Array.isArray(pilots?.data) ? pilots.data.length : 0;
    if (listCount > 0) {
      return listCount;
    }
  } catch {
...
async function updateBotPresenceWithPilots() {
  ...
  let pilotCount = Number.isFinite(lastKnownPilotCount) ? lastKnownPilotCount : 0;
  if (hasVamsysCredentials()) {
    const freshCount = await getRegisteredPilotsCount();
    if (Number.isFinite(freshCount) && freshCount >= 0) {
      pilotCount = freshCount;
      lastKnownPilotCount = freshCount;
    }
  }
    ...
    activities: [
      {
        ...
        name: `Serving for ${pilotCount} pilots | v0.2`,
      },
    ],
  });
}
...
client.once('clientReady', () => {
  console.log(`[READY] ✓ Bot is online: ${client.user.tag}`);
  console.log(`[READY] User ID: ${client.user.id}`);
  console.log(`[READY] Guilds: ${client.guilds.cache.size}`);
  console.log('[READY] Bot is ready to receive interactions...');
  ...
  notifyAutoPublishedContent().catch(() => null);
  if (contentAutoPublishTimer) {
    clearInterval(contentAutoPublishTimer);
  }
  contentAutoPublishTimer = setInterval(() => {
    notifyAutoPublishedContent().catch(() => null);
  }, contentAutoPublishIntervalMs);

  notifyPhotoLikes().catch(() => null);
  if (photoLikesNotificationTimer) {
    clearInterval(photoLikesNotificationTimer);
  }
  photoLikesNotificationTimer = setInterval(() => {
    notifyPhotoLikes().catch(() => null);
  }, 10 * 60 * 1000);

  notifyFlightCompletion().catch(() => null);
  if (flightCompletionNotificationTimer) {
    clearInterval(flightCompletionNotificationTimer);
  }
  flightCompletionNotificationTimer = setInterval(() => {
    notifyFlightCompletion().catch(() => null);
  }, 5 * 60 * 1000);

  notifyFlightReviewStatus().catch(() => null);
  if (flightReviewNotificationTimer) {
    clearInterval(flightReviewNotificationTimer);
  }
  flightReviewNotificationTimer = setInterval(() => {
    notifyFlightReviewStatus().catch(() => null);
  }, 5 * 60 * 1000);
});
...
client.on('interactionCreate', async (interaction) => {
  try {
    // DIAGNOSTIC: Log every interaction
    const cmdName = interaction.isChatInputCommand() ? interaction.commandName : 
                    interaction.isButton() ? `button:${interaction.customId}` :
                    interaction.isStringSelectMenu() ? `select:${interaction.customId}` :
                    interaction.isModalSubmit() ? `modal:${interaction.customId}` : 'unknown';
    console.log(`[INTERACTION] ${interaction.user.tag} triggered: ${cmdName}`);
  ...
  // DIAGNOSTIC: Log every slash command
  console.log(`[SLASH-COMMAND] /${interaction.commandName} from ${interaction.user.tag} in guild ${interaction.guild?.id || 'DM'}`);
  ...
  if (interaction.commandName === 'notam-create') {
    ...
    console.error(
      `[INTERACTION-ERROR] Handler failed (type: ${interaction?.type ?? 'unknown'}, command: ${interaction?.commandName || 'n/a'}, user: ${interaction?.user?.tag || 'unknown'}):`,
      error?.message || error
    );

    try {
      const fallbackMessage = 'Command failed while processing. Please try again.';
      ...
      console.error('[INTERACTION-ERROR] Failed to send error response:', replyError?.message || replyError);
    }
  }
});

async function bootstrap() {
  console.log('[BOOTSTRAP] Starting bot login...');
  ...
  console.log('[BOOTSTRAP] ✓ Client logged in, waiting for ready event...');

  if (!guildId) {
    console.warn(
      '[BOOTSTRAP] ⚠ DISCORD_GUILD_ID is not set. Commands are registered globally and may take time to appear.'
    );
  }

  const registered = await registerCommandsWithRetry({ attempts: 6, baseDelayMs: 5000 });
  if (!registered) {
    console.error(
      '[BOOTSTRAP] ✗ Bot is online, but slash command registration is currently failing. Existing commands will still work if they were already registered before.'
    );
  } else {
    console.log('[BOOTSTRAP] ✓ Commands registered successfully');
  }
}
