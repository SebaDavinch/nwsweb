...
**Сервер (`server/index.js`):**
- `ANTHROPIC_API_KEY` — новая env-переменная.
- `VA_AI_SYSTEM_PROMPT` — база знаний: флот, маршруты, вступление, ранги, события, NOTAM, документы, поддержка, TG-команды.
- `callClaudeAI(messages, lang)` — вызов `claude-sonnet-4-6`, детектирует `[ESCALATE]` в ответе.
- `createAiEscalationTicket()` — создаёт тикет без HTTP.
- `notifyAiEscalation()` — уведомляет TG-админов + Discord-embed при эскалации.
- **Telegram long-polling loop** `startTelegramPollingLoop()` — запускается при старте, `/start`/`/help` возвращают меню, текстовые сообщения → Claude → авто-ответ; эскалация создаёт тикет.
- `POST /api/ai/chat` — эндпоинт для сайта, поддерживает историю до 20 сообщений.

**Фронт (`ai-search-widget.tsx`):**
- Реальный вызов `/api/ai/chat` вместо regex.
- Анимированный typing-индикатор (`···`), кнопка «Обратиться за помощью», escalation success с номером тикета.

**Настройка на проде:** добавить `ANTHROPIC_API_KEY=sk-ant-...` в `.env`. Polling запустится автоматически.

---

### Диспетчерская — UX улучшения

- **Время вылета:** авто-подставляется текущее UTC + 40 минут при выборе рейса.
- **Карточки рейсов:** внизу карточки показываются модели ВС (`Boeing 737-82R`, `A321`) — теги из fleetIds.
- **Выбор ВС:** если на маршруте несколько типов — появляются чипы-фильтры (Все / тип), после выбора типа список показывает только регистрации этого типа.
- **Синхронизация локации:** кнопка ↺ рядом с ICAO пилота → `GET /api/pilot/location?force=true`, минует 10-минутный кэш, тянет актуальную локацию из vAMSYS.

### Rate limit vAMSYS — критический фикс

- `FLIGHT_MAP_CACHE_MS`: 100мс → **5000мс**. При 100мс живая карта делала 600 req/min при лимите 100 — блокировала все остальные вызовы.
- `apiFetch` / `apiRequest`: при 429 от vAMSYS — пауза 60с и retry вместо ошибки в UI.

**Релизы:** `website-release-20260620-180537.zip`, `website-release-20260620-182537.zip`, `website-release-20260620-195900.zip`
Изменялся `server/index.js` — нужен рестарт API после деплоя.
